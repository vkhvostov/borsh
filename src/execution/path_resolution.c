...
static char	*handle_absolute_path(char *command_name, int *exit_status)
{
	struct stat	path_stat;
	char		*abs_path;
...
static char	*try_path_component(const char *dir, size_t dir_len,
		const char *command_name, int *exit_status)
{
	char	*full_path;
	size_t	len;
	size_t	cmd_len;
...
char	*resolve_path(char *command_name, int *exit_status)
{
	...
	resolved_path = search_in_path(path_env_copy, command_name, exit_status);
	free(path_env_copy);
	return (resolved_path);
}
